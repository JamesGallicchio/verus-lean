...
through the Verus → VLIR → verus-lean → Boole → Strata pipeline, as of 2026-06-10.

Re-validated 2026-06-10 on `Strata` branch `pr/casts-boole` @ `fff49d4e3` (the
StrataBoole type-arg read-back fix below is carried in the working tree): the
working-suite gate holds 44 ✅ / 1 skip / 1 fail (`crypto_noref`), and every B1–B5
...
| 5 | `b5_minimal` | ✅ 16/0 | ✅ | ❌ trait associated type | — |
...
| 5 | `b5_minimal.lean:91` `Undeclared type or category Ops_Arith_mul_Output` | Trait associated type `<&MontgomeryPoint as Mul<&Scalar>>::Output` from the `MulSpecImpl` machinery is referenced generically but never resolved/declared. Needs associated-type resolution to the concrete impl type (here `MontgomeryPoint`). (Boole generation itself is clean — the tuple `IsVariant` crash from the bounds check is fixed; see the fixed-bugs table.) |
